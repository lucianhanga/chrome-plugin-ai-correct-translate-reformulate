...
| `scripting` | API | Programmatically inject the content script into the active tab when the user triggers an action (context menu or keyboard shortcut). The result overlay is always injected this way. | Declarative `content_scripts` for the overlay -- rejected because it would inject on every page load. The one exception is the selection toolbar, declared as a static content script but scoped to a narrow Outlook-host allowlist (see below) |
...
- [ ] Host permissions are broad: `host_permissions` is `<all_urls>` (required to inject the content script into cross-origin iframe editors). Compensating controls: egress is locked by the `connect-src` CSP to `localhost:11434` + `api.openai.com`; the result overlay is injected on-demand and programmatically (only on a context-menu click or keyboard shortcut); the sole static `content_scripts` entry (the selection toolbar) is scoped to a narrow Outlook-host allowlist, not `<all_urls>` (pinned by `tests/unit/manifest.test.ts`); the overlay uses a closed Shadow DOM. Narrowing `<all_urls>` is tracked in issue #44.
